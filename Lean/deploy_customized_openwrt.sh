...
# ==== [10.1] WireGuard 私钥注入（复制前在模板中替换占位符） ====
# 使用方式：在你的模板文件里把私钥写成占位符：__WG_PRIVKEY__
# 例：Lean/files/etc/config/network.Riviera 里：
...
if [[ "$inject_wgkey" =~ ^[Yy]$ ]]; then
  # 交互式输入（隐藏回显）+ 基本格式校验
  while true; do
    read -s -p "请输入 ${COMPILE_NAME} 的 WireGuard 私钥（典型44字符，末尾=）： " WG_PRIVKEY
    echo
  ...
  # 仅在临时克隆目录 $TMP_DIR 内替换，优先站点化文件，其次默认文件
  echo "🔎 正在扫描模板中的占位符 __WG_PRIVKEY__ ..."
  # 站点优先：*.${COMPILE_NAME}
  mapfile -t SITE_MATCHES < <(grep -RIl --null -e '__WG_PRIVKEY__' \
      "${TMP_DIR}/Lean" --include="*.${COMPILE_NAME}" 2>/dev/null || true)
  # 默认兜底：无后缀
  mapfile -t DEF_MATCHES < <(grep -RIl --null -e '__WG_PRIVKEY__' \
      "${TMP_DIR}/Lean" --exclude="*.${COMPILE_NAME}" 2>/dev/null || true)
