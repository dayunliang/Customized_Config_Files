...
if [ -f "$ADH_LOCAL_FILE" ]; then
  ...
  if [ -n "$(git status --porcelain "$REPO_FILE_NAME")" ]; then
    ...
    # 场景 C：git status 返回为空 -> 代表你本地运行的文件和刚克隆下来的云端文件连一个标点符号都不差！
...
# ------------------------------------------------------------------------------
# MosDNS 的规则属于频繁变动的分流清单（不涉及本地人工双向修改），
# 采用 wget 先下载至 .tmp 临时文件，校验成功后再瞬间覆盖的原子操作。
# ==============================================================================
...
download_files() {
  ...
  # 使用 while 循环逐行提取 URL 和保存文件名
  printf "%s\n" "$list" | while IFS=' ' read -r url fname; do
    [ -z "$url" ] && continue
    [ -z "$fname" ] && continue

    # 【防宕机原子下载】：下载目标设定为带有 .tmp 后缀的临时文件
    tmp_file="${target_dir}/${fname}.tmp"
    
    # -q: 静默模式，防止 crontab 邮箱被下载进度条塞满；
    # --timeout=15: 设置 15 秒网络超时，如果遇到墙或网络卡死，自动跳过该文件，绝对不让脚本挂起无限死锁！
    wget -q --timeout=15 "$url" -O "$tmp_file"

    # 【严格完整性校验】：检查命令执行返回码成功 ($? -eq 0) 且该临时文件确实非空、字节大于 0 ([ -s ])
    # 可以有效拦截由于网络 404 错误页面或者 GitHub 抽风吐出的 0 字节损坏文件！
    if [ $? -eq 0 ] && [ -s "$tmp_file" ]; then
      # 原子替换：只有新文件百分百验证完整，才会在 0.001 秒内覆盖正在使用的老规则
      mv "$tmp_file" "${target_dir}/${fname}"
    else
      # 异常清理：只要验证未通过，直接删掉 .tmp 垃圾文件，老文件继续稳定为 DNS 分流服务！
      rm -f "$tmp_file"
    fi
  done
}
...
# ------------------------------------------------------------------------------
# 针对不同 Linux 发行版或容器安装方式，自动探查可用的 Docker Compose 命令语法，
# 并根据前方对配置变化的状态追踪，决定是否重启容器。
# ==============================================================================
echo "=============================================================================="
